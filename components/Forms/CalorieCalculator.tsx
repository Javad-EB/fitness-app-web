...
import { convertInchesToCM, convertLbsToKg, getBmr } from "@/utils/calculations";
...
export default function CalorieCalculator() {
    ...
    const formik = useFormik({
        ...
        onSubmit: (values: {
            ...
            const metricWeight = units === "Imperial" ? convertLbsToKg(Number(weight)) : Number(weight)
            const metricHeight = units === "Imperial" ? convertInchesToCM(Number(height)) : Number(height)
    ...
    return (
        <div className="w-96 p-10">
            <form onSubmit={formik.handleSubmit}>
                ...
                <Input
                    className="mb-3"
                    label={`Height ${formik.values.units === "Metric" ? "(cm)" : "(inches)"
                        }`}
                ...
                <Input
                    className="mb-3"
                    label={`Weight ${formik.values.units === "Metric" ? "(kg)" : "(lbs)"
                        }`}
